...
(() => {
  const STRINGS = {
    ko: {
      ...
      presetDefault: '기본',
      paneSkillsH2: '스킬',
      hintSkills: '스킬은 이름+설명이 항상 시스템 프롬프트에 나열되고, 모델이 관련 있다고 판단하면 read_skill 도구로 아래 본문 전체를 불러옵니다. 프리셋과 달리 세션 전체 프롬프트를 바꾸지 않고, 필요할 때만 지침을 추가로 불러오는 방식입니다.',
      labelSkills: '스킬 목록',
      placeholderNewSkillName: '스킬 이름',
      placeholderNewSkillDesc: '한 줄 설명 (모델이 이걸 보고 선택)',
    ...
    en: {
      ...
      paneSkillsH2: 'Skills',
      hintSkills: "A skill's name+description are always listed in the system prompt; if the model decides one is relevant, it calls read_skill to load the full body below. Unlike a preset, this doesn't replace the whole session prompt — it adds instructions on demand.",
      labelSkills: 'SKILLS',
      placeholderNewSkillName: 'Skill name',
      placeholderNewSkillDesc: 'One-line description (the model picks based on this)',
