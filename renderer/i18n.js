...
(() => {
  const STRINGS = {
    ko: {
      ...
      labelLanguage: '언어',
      labelFontScale: '글자 크기',
      paneSystemH2: '시스템 프롬프트',
      labelSystemPrompt: '기본 SYSTEM PROMPT (프리셋을 지정하지 않은 세션에 적용)',
      placeholderSystemPrompt: '비워두면 시스템 프롬프트 없이 전송',
      labelPresets: '프리셋 (세션마다 다른 시스템 프롬프트를 지정할 때 사용)',
      placeholderNewPreset: '새 프리셋 이름',
      btnAddPreset: '+ 추가',
      noPresets: '아직 프리셋이 없습니다.',
      presetDefault: '기본',
      paneSkillsH2: '스킬',
      hintSkills: '스킬은 %APPDATA%/local-assistant/skills/<폴더>/SKILL.md 에서 읽어옵니다. 다른 곳(예: Claude Code용)에서 만든 SKILL.md 폴더를 그 안에 그대로 복사해 넣고 새로고침하면 아래 목록에 나타나며, "새 스킬"로 앱 안에서 직접 새로 작성할 수도 있습니다. 이름+설명은 항상 시스템 프롬프트에 나열되고, 모델이 관련 있다고 판단하면 read_skill 도구로 본문 전체를 불러옵니다.',
      labelSkills: '스킬 목록',
      placeholderSkillDesc: '한 줄 설명',
      placeholderSkillBody: '이 스킬이 로드됐을 때 모델에게 전달될 전체 지침',
      btnNewSkill: '+ 새 스킬',
      defaultNewSkillName: '새 스킬',
      btnRefreshSkills: '스킬 폴더 다시 스캔',
      ...
      labelMcpPerServer: 'SERVER당 검색 결과 수 (tool_search 한 번에 서버별로 반환할 도구 개수)',
      ...
      toolLabelWriteFile: '파일 쓰기 (UTF-8)',
      toolLabelGlobSearch: '파일 이름 검색',
      toolLabelGrepSearch: '내용 검색 (grep)',
      toolLabelToolSearch: '도구 검색 (MCP)',
      toolLabelReadSkill: '스킬 불러오기',
      toolLabelRunCommand: '⚠ 터미널 명령 실행 (임의 명령 실행 — 주의)',
      ...
      titleSessions: '세션 목록',
      titleSettings: '설정',
      titlePresetSelect: '이 세션의 시스템 프롬프트 프리셋',
      ...
      msgDeleteTitle: '이 대화 삭제 (질문과 그 답변을 함께 지웁니다)',
      msgRegenerateTitle: '이 질문부터 다시 생성 (이후 대화는 삭제됩니다)',
      confirmDeny: '거부',
      confirmApprove: '승인',
      confirmHead: '🔧 {name} 실행할까요?',
      ctxTokensSuffix: '{n} 토큰',
      activeMcpTools: '🔧 {n}',
      activeMcpToolsTitle: 'tool_search로 열린 MCP 도구 {n}개 — 이 세션의 모든 요청에 스키마가 실립니다:\n{list}',
      toolSearchNoHit: '일치하는 도구 없음',
    ...
    en: {
      ...
      labelSystemPrompt: 'DEFAULT SYSTEM PROMPT (used by sessions with no preset)',
      placeholderSystemPrompt: 'Leave empty to send no system prompt',
      labelPresets: 'PRESETS (per-session system prompts)',
      placeholderNewPreset: 'New preset name',
      btnAddPreset: '+ Add',
      noPresets: 'No presets yet.',
      presetDefault: 'Default',
      paneSkillsH2: 'Skills',
      hintSkills: 'Skills are read from %APPDATA%/local-assistant/skills/<folder>/SKILL.md. Copy a SKILL.md folder made elsewhere (e.g. for Claude Code) straight into that folder and refresh to see it below, or use "New skill" to author one directly in the app. A name+description is always listed in the system prompt; if the model decides one is relevant, it calls read_skill to load the full body.',
      labelSkills: 'SKILLS',
      placeholderSkillDesc: 'One-line description',
      placeholderSkillBody: 'Full instructions passed to the model once this skill is loaded',
      btnNewSkill: '+ New skill',
      defaultNewSkillName: 'New skill',
      btnRefreshSkills: 'Rescan skills folder',
      ...
      labelMcpServers: 'MCP SERVERS (JSON)',
      labelMcpPerServer: 'RESULTS PER SERVER (tools returned per server, per tool_search call)',
      ...
      toolLabelWriteFile: 'Write file (UTF-8)',
      toolLabelGlobSearch: 'Search file names',
      toolLabelGrepSearch: 'Search content (grep)',
      toolLabelToolSearch: 'Search tools (MCP)',
      toolLabelReadSkill: 'Load skill',
      toolLabelRunCommand: '⚠ Run terminal command (arbitrary execution — caution)',
      ...
      titlePresetSelect: "This session's system prompt preset",
      ...
      msgDeleteTitle: 'Delete this exchange (the question and its answer)',
      msgRegenerateTitle: 'Regenerate from this question (everything after it is deleted)',
      confirmDeny: 'Deny',
      confirmApprove: 'Approve',
      confirmHead: '🔧 Run {name}?',
      ctxTokensSuffix: '{n} tokens',
      activeMcpTools: '🔧 {n}',
      activeMcpToolsTitle: '{n} MCP tools opened via tool_search — their schemas ride along on every request in this session:\n{list}',
      toolSearchNoHit: 'no matching tools',
